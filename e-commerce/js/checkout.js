    ...
    .then(text => {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                container.innerHTML = '<div style="text-align: center; padding: 40px; color: #50c8aeff;">Error loading order summary: ' + (data.message || 'Unknown error') + '</div>';
            }
        } catch (e) {
            console.error('JSON Parse error:', e);
            console.error('Text was:', text);
            container.innerHTML = '<div style="text-align: center; padding: 40px; color: #50c8aeff;">Server response error. Please check console.</div>';
        }
    })
    .catch(error => {
        console.error('Fetch error:', error);
        container.innerHTML = '<div style="text-align: center; padding: 40px; color: #50c8aeff;">Failed to load order summary. Please check your connection.</div>';
    });
}
...
function displayCheckoutItems(items, total) {
    ...
    items.forEach((item, index) => {
        const imagePath = item.product_image ? 
            (item.product_image.startsWith('../') ? item.product_image : '/~lovette.philips/' + item.product_image) :
        ...
        itemDiv.innerHTML = `
            ...
            <div style="flex: 1;">
                ...
                <div style="font-size: 13px; color: #6b7280;">Qty: ${item.qty} × GHS ${parseFloat(item.price).toFixed(2)}</div>
            </div>
            <div style="font-weight: 700; color: #50c8aeff; font-size: 1.1rem;">
...
function createConfetti() {
    const colors = ['#50c8aeff', '#3a9682ff', '#16f9b1ff', '#fbbf24'];
...
function formatGhs(amount) {
    ...
    if (!isFinite(n) || isNaN(n)) return 'GHS 0.00';
    return 'GHS ' + n.toFixed(2);
}

// When rendering items / totals use formatGhs(item.subtotal) and formatGhs(total)
// Example snippet:
// const itemHtml = `
//   <div class="checkout-item">
//     <div class="qty">Qty: ${item.qty} ×</div>
//     <div class="title">${escapeHtml(item.title)}</div>
//     <div class="price">${formatGhs(item.subtotal)}</div>
//   </div>
// `;

// // Before sending amount to Paystack, 
// const totalNumber = Number(response.total);
// const safeTotal = (isFinite(totalNumber) && !isNaN(totalNumber)) ? totalNumber : 0.00;
// // Paystack expects amount in smallest currency unit (pesewas)
// const paystackAmount = Math.round(safeTotal * 100);
// fetch('/actions/paystack_init_transaction.php', {
//   method: 'POST',
//   headers: {'Content-Type': 'application/json'},
//   body: JSON.stringify({ amount: paystackAmount / 100, email: userEmail })
// });
